...
1. Copy the `.pte` files out of the Docker container:
   ...
   ```bash
   docker cp <container-id>:/home/ubuntu/executorch/model_u65.pte .
   docker cp <container-id>:/home/ubuntu/executorch/mobilenetv2_u65.pte .
   ```

   Replace `<container-id>` with your Docker container ID.

2. Write the `.pte` files to the first partition of the SD card so U-Boot can load them. The method depends on your operating system:
